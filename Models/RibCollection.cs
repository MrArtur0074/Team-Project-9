...
public class RibCollection
{
    ...
    /// </summary>
    public IReadOnlyList<double> Ribs { get => _ribs; }
    ...
    public bool AddRib(int index) {
       if (index > 0 && index < _ribs.Count) {
          double prevRibPos = _ribs[index - 1];
          double nextRibPos = _ribs[index];
          double space = (nextRibPos - prevRibPos) / 2.0;

          if (space + MathConstants.Tolerance < WingConstraints.MinInterRibSpace) {
             return false;
          }

          double position = prevRibPos + space;
    ...
    public bool ChangeRibPosition(int index, double shift) {
       ...
       double leftSpace = _ribs[index] - _ribs[index - 1];
       double rightSpace = _ribs[index + 1] - _ribs[index];
       double newLeftSpace = leftSpace + shift;
       double newRightSpace = rightSpace - shift;
       ...
       double newPosition = _ribs[index] + shift;
